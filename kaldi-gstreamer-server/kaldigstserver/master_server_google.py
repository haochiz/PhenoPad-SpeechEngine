...
import diarization_test as dia_test
import speaker_embedding_worker_new_2 as embedding_worker

#import mic_vad_streaming_2 as deepspeech

import webrtcvad
import collections
import wave

#DATA_PATH = '/home/haochi/speechengine_test/data/'
DATA_PATH = './data/'
...
class WorkerSocketHandler(tornado.websocket.WebSocketHandler):
        ...
        if 'result' in event:
            ...
            if not event['result']['final']:
                try:                                                    
                    ...
                    #event = self.get_gg_diarization_result(event)
                    event = self.get_empty_diarization_result(event)
    ...
    def get_gg_diarization_result(self, event):
        event['result']['diarization'] = self.gg_dia_segs
        event['result']['diarization_incremental'] = False
        return event

    def send_gg_diarization_result(self):
        event = {'status': 0, 'segment': 0, 'result': {'hypotheses': [{'transcript': '...'}], 'final': False}, 'id': ''}
        event['result']['diarization'] = self.gg_dia_segs
        event['result']['diarization_incremental'] = False
        self.client_socket.send_event(event)

    def get_empty_diarization_result(self, event):
        event['result']['diarization'] = []
        event['result']['diarization_incremental'] = True
        return event
...
class DecoderSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def open(self):
        ...
        self.segmentation_method = "VAD" # choose between {ODAS, VAD, and NONE?}
        ...
        #self.temp_audio_file = open('./temp_audio_file.raw', 'ab')
        dt = datetime.datetime.now() 
        ...
        # worker might not be ready when client initiates request
        #self.obtain_worker()
        #logging.info("%s: client connection opened" % self.id)
        #self.application.active_clients.append(self)   
    ...
    # google speech-to-text
    def gg_start(self):
        threading.Thread(target=self.gg_process).start()
    
    def gg_process(self):
        gg_client = speech.SpeechClient()
        
        dia_config = speech.types.SpeakerDiarizationConfig(
        ...
        audio_generator = self.gg_stream_generator()
        requests = (types.StreamingRecognizeRequest(audio_content=content)
                    for content in audio_generator)

        responses = gg_client.streaming_recognize(streaming_config, requests)
        try:
            self.gg_response_loop(responses)
        except:
            self.gg_start()

    def gg_stream_generator(self):
        while True:
            chunk = self.gg_audio_queue.get()
            if chunk is None:
                return
            data = [chunk]
            while True:
                try:
                    chunk = self.gg_audio_queue.get(block=False)
                    if chunk is None:
                        return
                    data.append(chunk)
                except queue.Empty:
    ...
    def gg_response_loop(self, responses):
        """
        ...
        for response in responses:
            ...
            if result.is_final:
                ...
                message = self.format_gg_results_final(result)
                self.worker.client_socket.send_event(message)
                self.worker.gg_dia_segs = self.process_gg_diarization(words)
                self.worker.send_gg_diarization_result()
            else:
                message = self.format_gg_results_partial(result)

    def format_gg_results_partial(self, result):
        #result = response.results[0]
        
        hypo_dict = {'transcript': result.alternatives[0].transcript}

        dia_segs = []

        message = {'status': 0, 'segment': 0, 'result': {'hypotheses': [hypo_dict],\
                   'diarization': dia_seg, 'final': True, 'diarization_incremental': True},\
                   'id': 'google-partial'}
    ...
    def format_gg_results_final(self, result):
        #print(response.results[0])
        #print(response.results[0].alternatives[0].words)
        #result = response.results[0]

        words = result.alternatives[0].words

        # get new segment words (for when using diarization)
        new_words_count = len(words) - self.total_words
        self.total_words = len(words)
        words = words[-new_words_count:]

        message = {}
        
        seg_start = self.format_gg_time(words[0].start_time)
        seg_end = self.format_gg_time(words[-1].end_time)
        seg_length = round(seg_end - seg_start, 1)
        total_length = self.format_gg_time(result.result_end_time)
        
        seg_id = self.gg_seg_id
        self.gg_seg_id += 1
        
        seg_words = self.format_gg_words(words)
        hypo_dict = {'transcript': result.alternatives[0].transcript,\
                     'confidence': result.alternatives[0].confidence, 'likelihood': 0.0,\
                     'word-alignment': seg_words}

        message = {'status': 0, 'segment-start': seg_start, 'segment-length': seg_length,\
                'total-length': total_length, 'result': {'hypotheses': [hypo_dict], 'final': True},\
                'segment': seg_id, 'id': 'google-final-'+str(seg_id)}
        
        #print(message)
        return message

    def format_gg_words(self, words):
        output = []
        for i in range(len(words)):
            w_start = self.format_gg_time(words[i].start_time)
            w_end = self.format_gg_time(words[i].end_time)
            w_length = round(w_end-w_start, 1)
            # set confidence = 1.0 since google doesn't provide word level conf
            w_dict = {'start': w_start, 'length': w_length, 'word': words[i].word, 'confidence': 1.0}
            output.append(w_dict)
        return output

    def format_gg_time(self, time):
        if not time:
            return 0.0
        return time.seconds + float(time.nanos)*1e-9

    def process_gg_diarization(self, words):
        print('#############')
        try:
            ...
            for i in range(len(words)):
                w_start = self.format_gg_time(words[i].start_time)
                w_end = self.format_gg_time(words[i].end_time)
                last_w_end = self.format_gg_time(words[i-1].end_time)
                
                if not current_speaker: # first word
                    current_speaker = words[i].speaker_tag
                    current_seg_start = w_start
                    continue
                if words[i].speaker_tag != current_speaker: # speaker change
                    segs.append({'start': current_seg_start, 'end': last_w_end, 'speaker': current_speaker-1})
                    current_speaker = words[i].speaker_tag
                    current_seg_start = w_start
                if i == len(words) - 1: # last word
                    segs.append({'start': current_seg_start, 'end': w_end, 'speaker': current_speaker-1})
    ...
    def on_connection_close(self):
        ...
        # self.write_message('CLOSE') this will cause error
        if self.worker:
            try:
                self.worker.set_client_socket(None)
            ...
            if self.DECODER_STATUS == "OPEN":
                if isinstance(message, unicode):
                    ...
                    if self.external == False:
                        ...
                        # google speech to text
                        self.gg_audio_queue.put(message)
                                    ...
                                    if not self.triggered:
                                        ...
                                        if num_unvoiced > 0.5 * self.ring_buffer.maxlen:
                                            ...
                                            if self.segmentation_method == "VAD":
                                                ...
                                                self.beamforming_result_queue.put(VAD_seg)
                        ...
                        if self.segmentation_method != None:  
                            # TODO: merge this function with the "surface mode" code
                            # VAD segmentation
                            self.vad_segment(message_short)
        except Exception, e:
    ...
    def start_odas_process(self):
        ...
        #TODO: do something about this in the future?
        self.odas_dead = True
        self.close_diarization_process()
    ...
    def send_audio_to_odas(self, message):
        try:
            ...
            print(traceback.format_exc())
            self.odas_dead = True
            #self.worker.client_socket.write_message('ODASERROR')
            #self.write_message("ODAS_ERROR")
    ...
    def close_diarization_process(self):
        try:
            if self.segmentation_method == "ODAS":
                ...
                self.sst_sock.close()
            self.diarization_process.terminate()
            self.embedding_process.terminate()
            #self.odas_sst_queue.close()    sst queue already gone at this line, will raise exception
            #del self.odas_sst_queue
        except:
    ...
    def vad_segment(self, message_short):
    ...
    def start_diarization_process(self):
        ...
        self.beamforming_result_queue = multiprocessing.Queue()
        #TODO: add clearer documentation, change function names to reflect what they actually do
        #TODO: this is skipped if not using ODAS, VAD directly pushes segments to beamforming_result_queue
        if self.external:
            self.sst_trans = open("sst_trans.txt", "w")
            self.diarization_process = multiprocessing.Process(target = dia_test.start_diarization, \
                                                               args = (self.sst_queue, \
                                                               self.beamforming_result_queue))
            #self.diarization_process.start()

        self.embedding_process = multiprocessing.Process(target = embedding_worker.start_embedding, \
                                                           args = (self.beamforming_result_queue, \
                                                                   self.worker.diarization_result_queue))
        #self.embedding_process.start()
    
    #NOTE: not used
    def start_diarization_non_external(self):
        """ start diarization subprocess 
        """
        #TODO: merge with start_diarization_process
        self.worker.diarization_result_queue = multiprocessing.Queue()
        self.beamforming_result_queue = multiprocessing.Queue()
        self.embedding_process = multiprocessing.Process(target = embedding_worker.start_embedding, \
                                                           args = (self.beamforming_result_queue, \
...
def main():
    ...
    #print("listening from port: ", options.port)
    else:
