...
class FewShotSpeech(nn.Module):
    ...
    def loss_various_len(self, batch):
        xq_padded = batch['xq_padded'] # n_class * n_query * max_len * mfcc_dim
        xs_padded = batch['xs_padded'] # n_class * n_support * max_len * mfcc_dim
        xq_len = batch['xq_len'] # n_class * n_query 
        xs_len = batch['xs_len'] # n_class * n_support
        
        assert xq_padded.shape[0] == xq_len.shape[0]
        assert xs_padded.shape[0] == xs_len.shape[0]
        ...
        xq_len, xq_perm_idx = s.sort(0, descending=True)
