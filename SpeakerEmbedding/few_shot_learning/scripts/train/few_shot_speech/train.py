...
#sh = logging.FileHandler('train.log')
...
def main(opt):
    ...
    opt['model.in_dim'] = list(map(int, opt['model.in_dim'].split(',')))
    ...
    if opt['train.checkpoint'] is None:
        logger.info('Training model from scratch...')
        model = model_utils.load(opt)
    else:
        logger.info('Loading model from checkpoint...')
        model = torch.load(opt['train.checkpoint'])
    ...
    def on_start(state):
        ...
        if opt['train.checkpoint'] is None:
            ...
            # optim = torch.load(opt['train.checkpoint'])['optimizer']
            # state['optimizer'] = optim
            state['scheduler'] = lr_scheduler.StepLR(state['optimizer'], opt['train.decay_every'], gamma=0.5)
